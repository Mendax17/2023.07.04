...
x_train=np.load(data_path+'brain_x_train.npy')
y_train=np.load(data_path+'brain_y_train.npy')

x_test=np.load(data_path+'brain_x_test.npy')
y_test=np.load(data_path+'brain_y_test.npy')

print(x_train.shape, y_train.shape) # (160, 200, 200, 1) (160, )
print(x_test.shape, y_test.shape) # (120, 200, 200, 1) (120, )

x_test, x_val, y_test, y_val = train_test_split(x_test, y_test, train_size=0.8, shuffle=True, random_state=333)
...
hist=model.fit(x_train, y_train, epochs=1, batch_size=8, validation_data=(x_val, y_val))
