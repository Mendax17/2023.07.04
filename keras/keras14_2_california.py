...
model=Sequential()
model.add(Dense(30, input_dim=8)) #x의 덩어리 갯수 #행렬에서 열의 갯수와 같다 #열의 갯수가 우선된다 #열=컬럼,피처,특성 을 의미 # ex)환율,금리,물가지수 이런 요소들이 열
model.add(Dense(25))
model.add(Dense(20))
model.add(Dense(15))
model.add(Dense(14))
...
# 0.51 # state=50 #size=8
# 0.53 # state=60 #size=8
# 0.544 # state=70 #size=8
# 0.55 # state=70 #size=8 # epochs=150
# 0.53 # state=70 #size=4