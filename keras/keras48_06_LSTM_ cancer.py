...
model.add(LSTM(units=32, input_shape=(30, 1), activation='linear'))#, return_sequences=True)) # (N, 64) 3차원으로 받아야하는데 LSTM의 결과값은 2차원으로 나오기 때문에 그 다음에 LSTM layer를 놓으면 ERROR가 발생
...
mcp=ModelCheckpoint(monitor='val_loss', mode='auto', verbose=1, save_best_only=True, filepath=save_mcp_path+'k48_06_cancer_'+save_name)
...
# accuarcy_score : 0.9485294117647058