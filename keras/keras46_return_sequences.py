...
model.add(LSTM(units=64, input_shape=(3, 1), return_sequences=True)) # (N, 64) 3차원으로 받아야하는데 LSTM의 결과값은 2차원으로 나오기 때문에 그 다음에 LSTM layer를 놓으면 ERROR가 발생
...
model.fit(x, y, epochs=500, batch_size=1, validation_split=0.2, verbose=3, callbacks=[es])#, mcp])
