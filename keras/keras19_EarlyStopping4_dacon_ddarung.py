...
#2. 모델
model=Sequential()
model.add(Dense(64, input_shape=(9, ), activation='linear')) # 앞으로 다차원이 나오게 되면 input_dim이 아닌 input_shape로 하게된다. # input_dim=9과 같은 의미
...
hist=model.fit(x_train, y_train, epochs=300, batch_size=8, validation_split=0.25, verbose=1, callbacks=[earlyStopping]) # train 데이터의 75%를 train으로 사용하고 25% validation 데이터로 사용하겠다는뜻
