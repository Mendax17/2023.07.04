...
# 피쳐를 2로 바꿀거야
x_train=x_train.reshape(-1, 2, 2) # (72, 2, 2)
x_test=x_test.reshape(-1, 2, 2) # (24, 2, 2)
x_predict=x_predict.reshape(-1, 2, 2) # (7, 2, 2)

print(x_train.shape, x_test.shape, x_predict.shape)

#2. 모델구성
model=Sequential() # (N, 4, 1) units의 64가 1의 dim 부분에 들어간다.
model.add(LSTM(units=32, input_shape=(2, 2)))#, return_sequences=True)) # (N, 64) 3차원으로 받아야하는데 LSTM의 결과값은 2차원으로 나오기 때문에 그 다음에 LSTM layer를 놓으면 ERROR가 발생
