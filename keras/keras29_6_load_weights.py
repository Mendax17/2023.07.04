...
# 가중치 저장 1번 위치
#model.save_weights(path+'keras29_5_save_weights1.h5') # save_weights는 모델 저장 기능은 없기 때문에 모델 정의가 있어야한다.

# 1번 가중치 저장값 불러오기
#model.load_weights(path+'keras29_5_save_weights1.h5') # RuntimeError: You must compile your model before training/testing. Use `model.compile(optimizer, loss) # 가중치 저장이 안되어있는걸 확인할 수 있다.
...
# 2번 가중치 저장값 불러오기
model.load_weights(path+'keras29_5_save_weights2.h5') # RuntimeError: You must compile your model before training/testing. Use `model.compile(optimizer, loss) # weight가 있어도 compile이 정의가 안되어있으면 ERROR가 발생한다.
# 순수하게 weights 값만 가지고 있기 때문에 model과 compile이 정의되어 있어야 사용할 수 있다.
