...
path='c:/Users/eagle/Downloads/bitcamp/_data/ddarung/'
...
train_csv['date_year'] = train_csv.date_time.apply(lambda t : t.year)
test_csv['date_year'] = test_csv.date_time.apply(lambda t : t.year)
train_csv['month_day'] = train_csv.date_time.apply(lambda t : t.strftime('%m-%d'))

plt.figure(figsize = (20, 12))
sns.lineplot(x = 'month_day', y = 'number_of_rentals', data = train_csv, hue = 'date_year')
...
#2. 모델구성
model=Sequential()
model.add(Dense(32, input_shape=(9, ), activation='relu')) # 앞으로 다차원이 나오게 되면 input_dim이 아닌 input_shape로 하게된다. # input_dim=13과 같은 의미
...
hist=model.fit(x_train, y_train, epochs=1000, batch_size=8, validation_data=(x_val, y_val), verbose=1, callbacks=[earlyStopping]) # model.fit의 어떤 반환값을 hist에 넣는다.
