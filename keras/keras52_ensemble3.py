...
y3=np.array(range(201, 301)) # (100, )


x1_train, x1_input, x2_train, x2_input, x3_train, x3_input, y1_train, y1_input, y2_train, y2_input, y3_train, y3_input = \
    train_test_split(x1_datasets, x2_datasets, x2_datasets, y1, y2, y3, train_size=0.8, shuffle=True, random_state=333)
x1_test, x1_val, x2_test, x2_val, x3_test, x3_val, y1_test, y1_val, y2_test, y2_val, y3_test, y3_val = \
    train_test_split(x1_input, x2_input, x3_input, y1_input, y2_input, y2_input, train_size=0.8, shuffle=True, random_state=333)
...
merge1=Concatenate()([output1, output2, output3]) # 클래스 Concatenate는 ()를 추가하면 된다.
...
#2-4. 모델분리-3
dense61=Dense(21, activation='linear')(last_output)
dense62=Dense(22, activation='linear')(dense61)
output6=Dense(23, activation='linear')(dense62)

model = Model(inputs=[input1, input2, input3], outputs=[output4, output5, output6]) # outputs을 2개로 지정해줘야 한다.
...
mcp=ModelCheckpoint(monitor='val_loss', mode='auto', verbose=1, save_best_only=True, filepath=save_mcp_path+'k52_ensemble3_'+save_name)
model.fit([x1_train, x2_train, x3_train], [y1_train, y2_train, y3_train], epochs=1000, batch_size=32, validation_data=([x1_val, x2_val, x3_val],[y1_val, y2_val, y3_val]), verbose=3, callbacks=[es, mcp])

#4. 평가, 예측
loss=model.evaluate([x1_test, x2_test, x3_test], [y1_test, y2_test, y3_test])
print('loss :', loss)
# loss : [1121.095458984375, 36.41965103149414, 541.2476806640625, 543.4281005859375]
# 1121.0=36.4+541.2+543.4
# 앙상블 모델에서는 메인로스 1개(각각의 모델 로스의 합)와 합쳐진 모델 3개의 각각의 로스 총 4개가 출련된다.
