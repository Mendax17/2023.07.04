...
x_train, x_test, y_train, y_test = train_test_split(x, y, train_size=0.8, random_state=40) # train_test_split는 2차원만 받아들일 수 있다. # train_size의 default는 0.75

print(x_train.shape, x_test.shape, x_predict.shape)

#2. 모델구성
model=Sequential() # (N, 4, 1) units의 64가 1의 dim 부분에 들어간다.
model.add(Dense(units=32, input_shape=(4, )))#, return_sequences=True)) # (N, 64) 3차원으로 받아야하는데 LSTM의 결과값은 2차원으로 나오기 때문에 그 다음에 LSTM layer를 놓으면 ERROR가 발생
...
model.fit(x_train, y_train, epochs=1000, batch_size=16, validation_split=0.2, verbose=3, callbacks=[es])#, mcp])
