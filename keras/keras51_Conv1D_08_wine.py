...
#2. 모델구성
model=Sequential()
model.add(Conv1D(32, kernel_size=2, input_shape=(13, 1), activation='linear'))
model.add(Dropout(0.1))
model.add(Conv1D(64, kernel_size=2, activation='linear'))
model.add(Dropout(0.1))
model.add(Conv1D(32, kernel_size=2, activation='linear'))
model.add(Dropout(0.1))
model.add(Conv1D(16, kernel_size=2, activation='linear'))
model.add(Flatten())
...
mcp=ModelCheckpoint(monitor='val_loss', mode='auto', verbose=1, save_best_only=True, filepath=save_mcp_path+'k51_Conv1D_08_wine_'+save_name)
...
# accuarcy_score : 0.9767441860465116