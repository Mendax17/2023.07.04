...
#2. 모델
model=Sequential()
model.add(Dense(32, input_shape=(10, ), activation='linear')) # 앞으로 다차원이 나오게 되면 input_dim이 아닌 input_shape로 하게된다. # input_dim=10과 같은 의미
model.add(Dense(64))
model.add(Dense(32))
model.add(Dense(64))
model.add(Dense(32))
...
hist=model.fit(x_train, y_train, epochs=400, batch_size=4, validation_split=0.2, verbose=1, callbacks=[earlyStopping]) # train 데이터의 75%를 train으로 사용하고 25% validation 데이터로 사용하겠다는뜻
