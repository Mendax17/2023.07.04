...
#2. 모델
model=Sequential()
model.add(Conv2D(filters=128, kernel_size=(3, 3), input_shape=(28, 28, 1), padding='same', strides=3, activation='relu')) # (28, 28, 1)
model.add(MaxPooling2D()) # (28, 28) 짜리를 (2, 2)인 pool_size로 strdies=2 인 상태로 처리 되므로 절반인 (14, 14)이 된다.
model.add(Conv2D(filters=64, kernel_size=(2, 2), padding='same', activation='relu')) # (26, 26, 64)
...
mcp=ModelCheckpoint(monitor='val_loss', mode='auto', verbose=1, save_best_only=True, filepath=save_mcp_path+'k34_'+save_name)#filepath=path+'MCP/keras30_ModelCheckPoint3.hdf5')
